...
function aiAnalyze() {
  ...
  st.openCursor().onsuccess = (e) => {
    ...
    if (c) { items.push(c.value); c.continue(); }
      ...
      const latestValue = filtered[0];  // The latest measured value
      const prevValues = filtered.slice(1); // Previous values for trend calculation

      // Calculate trend (slope) based on the previous values
      const s = statsAndTrend(prevValues);
      const tooHigh = latestValue.glucose > s.max;
      const tooLow = latestValue.glucose < s.min;
      const isInRange = !tooHigh && !tooLow;

      // Immediate feedback on the current value based on trend
      lines.push(`${ICON.pin} Vaša poslednja vrednost: ${latestValue.glucose.toFixed(1)} mmol/L.`);

      if (isInRange) {
        lines.push(`${ICON.ok} Ova vrednost je u skladu sa referentnim opsezima.`);
      } else if (tooHigh) {
        lines.push(`${ICON.warn} Vrednost je previsoka, obratite pažnju na ishranu i terapiju.`);
      } else if (tooLow) {
        lines.push(`${ICON.warn} Vrednost je preniska, obratite pažnju na ishranu i terapiju.`);
      }

      // Now showing trend based on previous values (no longer using average)
      if (s) {
        if (s.slope > 0.1) lines.push(`${ICON.warn} Trend rasta u poslednjim merenjima.`);
        if (s.slope < -0.1) lines.push(`${ICON.ok} Vrednosti opadaju — odličan napredak!`);

        lines.push(`Trenutni trend: ${s.slope > 0 ? "rast" : (s.slope < 0 ? "pad" : "stagnacija")}.`);
      }

      // Additional advice based on context
      if (tooHigh || tooLow) {
        lines.push(`${ICON.doctor} Ako se obrazac nastavi, konsultujte lekara.`);
      } else {
        lines.push(`${ICON.ok} U okviru ste ciljeva za ovaj period/kontekst.`);
      }

      lines.push(`Napomena: Ovo nije medicinski savet.`);
      showAItyping(lines);
    }
  };
}
  ...
  st.openCursor().onsuccess = (e) => {
    ...
    if (c) { items.push(c.value); c.continue(); }
      ...
      // Immediate feedback on the current value
      lines.push(`${ICON.pin} Vaša poslednja vrednost: ${latestValue.glucose.toFixed(1)} mmol/L.`);
    ...
    if (c) { items.push(c.value); c.continue(); }
      ...
      for (const b of blocks) {
        ...
        if (b.key === 'morning' && (tooHigh || s.max > b.refDia.high)) {
          lines.push(`   ℹ️ Mogući “dawn phenomenon”: jutarnji hormoni (kortizol, hormon rasta) mogu podizati šećer.`);
        }
        if (tooHigh || tooLow) {
          lines.push(`   ${ICON.doctor} Ako se ovakav obrazac nastavi, konsultujte lekara.`);
        }
      }
