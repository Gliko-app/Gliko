...
function initUI() {
  ...
  // Dodajemo event listener za taster "AI analiza cele tabele"
  document.getElementById('btnAnalyzeTable').addEventListener('click', function() {
    aiAnalyzeTable();  // Pozivamo funkciju aiAnalyzeTable
  });
}
...
function addEntry() {
  ...
  const glucoseInput = byId('glucose').value.trim();  // Preuzimanje vrednosti unosa glukoze
  const glucose = parseFloat(glucoseInput);  // Parsiramo unos glukoze u broj
  ...
  console.log("Unos glukoze:", glucoseInput);  // Logovanje unosa
  console.log("Parsed vrednost glukoze:", glucose);  // Logovanje parsed vrednosti glukoze

  // Proveriti da li je unos glukoze validan
  if (!date || !time || glucoseInput === '' || isNaN(glucose) || glucose <= 0) { 
    alert('Datum, vreme i glukoza su obavezni.');
    return;
  }
  ...
  tx.oncomplete = () => {
    ...
    // Pokrećemo analizu odmah nakon unosa, prosleđujući vrednost glukoze
    aiAnalyze(glucose);  // Pozivanje analize i prikazivanje modala
  };
}
