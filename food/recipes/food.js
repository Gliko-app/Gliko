...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch('recipes.json')
    ...
    .then(recipes => {
      ...
      recipes.forEach(recipe => {
        ...
        card.innerHTML = `
          <img src="${recipe.slika}" alt="${recipe.naziv}"> <!-- Slika recepta -->
  ...
  function openRecipeModal(recipe) {
    ...
    if (!recipeTitle || !recipeImage || !recipeIngredients || !recipePreparation || !recipeCalories) {
      console.error("Jedan od modala za recept nije pronađen!");
      return;
    }

    recipeTitle.textContent = recipe.naziv;
    recipeImage.src = recipe.slika; // Dodajemo sliku u modal
    recipeIngredients.textContent = recipe.sastojci;
    recipePreparation.textContent = recipe.priprema;
    recipeCalories.textContent = recipe.kalorije;
    recipeModal.hidden = false; // Prikazujemo modal
  }

  // Zatvori modal za recept
  if (closeRecipeModal) {
